...
def generate_web(evs, arbs):
    ...
    def build_ev_card(e):
        live_badge = '<span style="color:#ef4444; font-weight:bold; animation: pulse 1.5s infinite;">🔴 LIVE</span>' if e.get('is_live') else f"📅 {e['time']}"
        return f"""
    ...
    def build_arb_card(a):
        live_badge = '<span style="color:#ef4444; font-weight:bold; animation: pulse 1.5s infinite;">🔴 LIVE</span>' if a.get('is_live') else f"📅 {a['time']}"
        legs_html = ""
        for s in a['sides']:
            legs_html += f"""
    ...
    pre_ev_html = "".join([build_ev_card(e) for e in evs if not e.get('is_live')][:50]) or "<div style='text-align:center; padding:40px; color:#a1a1aa;'>No Pre-Match EV detected.</div>"
    live_ev_html = "".join([build_ev_card(e) for e in evs if e.get('is_live')][:50]) or "<div style='text-align:center; padding:40px; color:#a1a1aa;'>No Live EV detected.</div>"
    pre_arb_html = "".join([build_arb_card(a) for a in arbs if not a.get('is_live')][:50]) or "<div style='text-align:center; padding:40px; color:#a1a1aa;'>No Pre-Match Arbitrage locks.</div>"
    live_arb_html = "".join([build_arb_card(a) for a in arbs if a.get('is_live')][:50]) or "<div style='text-align:center; padding:40px; color:#a1a1aa;'>No Live Arbitrage locks.</div>"
...
<div class="tabs">
    <div class="tab active" onclick="showPane('pre-ev')">💎 Pre-Match EV</div>
    <div class="tab" onclick="showPane('live-ev')">🔴 Live EV</div>
    <div class="tab" onclick="showPane('pre-arb')">🔒 Pre-Match ARB</div>
    <div class="tab" onclick="showPane('live-arb')">🔴 Live ARB</div>
...
if __name__ == "__main__":
    ...
    if bc_data:
        print(f"\n🔄 Attempting to merge {len(bc_data)} BC.Game matches with Global Market...")
        merge_count = 0
    ...
    for a in arbs:
        ...
        if not is_duplicate_alert(alert_key):
            msg = f"🏆 {a['sport']}\n📈 {a['line']} ({a['ways']}-Way)\n\n"
            for s in a['sides']: msg += f"🔵 ₹{s['stk']:.0f} on {s['sel'].upper()} @ {s['pr']:.2f} [{s['bk'].title()}]\n"
            msg += f"\n✨ Profit: ₹{a['profit']:.0f}"
            try:
                requests.post("https://ntfy.sh/", json={"topic": NTFY_CHANNEL, "message": msg, "title": f"🚨 {a['pct']:.2f}% ARB | {a['match']}", "tags": ["moneybag"]}, timeout=5)
            except: pass
    ...
    for e in evs:
        ...
        if not is_duplicate_alert(alert_key):
            ...
            try:
                requests.post("https://ntfy.sh/", json={"topic": NTFY_CHANNEL, "message": msg, "title": f"📈 {e['pct']:.2f}% EV | {e['match']}", "tags": ["chart_with_upwards_trend"]}, timeout=5)
            except: pass
