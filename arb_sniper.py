...
function fireEVAlert(ev){
  ...
  var body=ev.sport.replace(/_/g,' ').toUpperCase()+' · '+ev.time
    +'\n\n💰 BET: '+ev.sel.toUpperCase()+' @ '+ev.odds.toFixed(2)+' on '+bk(ev.bk)
    +'\n📐 Kelly Stake: '+fmt(ev.stk)
    +'\n🧠 True Odds: '+ev.trueO.toFixed(3)+' | Conf: '+ev.conf+'/100';
  sendNtfy(title,body);
}
...
function fireARBAlert(arb){
  ...
  arb.sides.forEach(function(s){body+='\n🔵 '+fmt(s.stk)+' on '+s.sel.toUpperCase()+' @ '+s.pr.toFixed(2)+' ['+bk(s.bk)+']'});
...
function persistData() {
  localStorage.setItem('arb_data_ev', JSON.stringify(STATE.evs));
  localStorage.setItem('arb_data_arb', JSON.stringify(STATE.arbs));
  localStorage.setItem('arb_data_net', JSON.stringify(STATE.keyStats));
  localStorage.setItem('arb_data_ts', Date.now());
}

function restoreData() {
  const ts = localStorage.getItem('arb_data_ts');
  if(ts && (Date.now() - parseInt(ts) < 3600000)) { 
    try {
      STATE.evs = JSON.parse(localStorage.getItem('arb_data_ev') || '[]');
      STATE.arbs = JSON.parse(localStorage.getItem('arb_data_arb') || '[]');
      const savedNet = JSON.parse(localStorage.getItem('arb_data_net'));
      if(savedNet) STATE.keyStats = savedNet;
      
      if(STATE.evs.length || STATE.arbs.length) {
        $('ev-cards').innerHTML = ''; $('arb-cards').innerHTML = '';
        STATE.evs.forEach((e,i) => renderEV(e,i));
        STATE.arbs.forEach((a,i) => renderARB(a,i));
        updateBadges(); renderNetwork(); renderAn();
        log('🔄 Restored data from previous scan session.', 'linf');
        return true;
      }
    } catch(e) { console.error("Cache corrupted, skipping restore."); }
  }
  return false;
}
...
async function runNext() {
  ...
  STATE.sportIdx++;
  if(STATE.scanning) setTimeout(runNext, 1500); 
}
    ...
    (ev.bookmakers||[]).forEach(b => {
      (b.markets||[]).forEach(m => {
        m.outcomes.forEach(o => {
          let pr = o.price;
          if(b.key === 'betfair_ex_eu') pr = 1 + (pr-1)*0.97; 
...
function toggleFilter(f) {
  ...
  $(f+'Btn').style.borderColor = STATE.filters[f] ? 'var(--cyan)' : 'var(--b2)';
  applyFilters();
}
...
function renderAn() {
  ...
  const a1 = a.filter(x=>x.pct<2).length, a2 = a.filter(x=>x.pct>=2).length;
  $('anAH').innerHTML = `<div class="hrow"><span class="hlbl">< 2%</span><div class="htrack"><div class="hbar" style="width:${a1?a1/a.length*100:0}%;background:var(--purple)"></div></div><span class="hcnt">${a1}</span></div><div class="hrow"><span class="hlbl">> 2%</span><div class="htrack"><div class="hbar" style="width:${a2?a2/a.length*100:0}%;background:var(--cyan)"></div></div><span class="hcnt">${a2}</span></div>`;
}
