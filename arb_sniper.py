...
def fetch_bcgame_events() -> list:
    ...
    try:
        r = requests.get(f"{BCGAME_BASE}/0", headers=headers, timeout=20)
        if r.status_code != 200: return []
        manifest = r.json()
        ...
        for chunk_id in all_ids[:5]: # Limit to 5 chunks for speed
            chunk = requests.get(f"{BCGAME_BASE}/{chunk_id}", headers=headers, timeout=10).json()
...
def scan_arbitrage(events: list) -> list:
    ...
    for ev in events:
        ...
        for mkey in MARKETS:
            ...
            # Simple 2-Way or 3-Way combinations
...
function triggerPaymentGateway() {{
    ...
    // The backend webhook is expected to listen for this exact decimal amount
    ...
    // Zero-Trust Polling: We poll the user's DB entry waiting for the BACKEND to update it
...
function renderAnalytics(){{
    // Total Arb Profit
    let totP = 0;
    ARBS.forEach(a => totP += ((a.profit_pct/100) * 10000)); // Standardized 10k baseline
    ...
    // Top Bookmakers by EV Opportunity
    let bCounts = {{}};
